...
export default function DashboardShell({ items, title, children }) {
  ...
  return (
    <div className="min-h-screen flex bg-stone-50">
      ...
      <aside
        className={`w-64 shrink-0 bg-white border-r border-stone-200 flex flex-col h-full z-50
      ...
      <div className="flex-1 min-w-0 flex flex-col lg:ml-0">
        <header className="lg:hidden h-16 bg-white border-b border-stone-200 px-4 flex items-center justify-between sticky top-0 z-30">
          <Button variant="ghost" size="icon" onClick={() => setOpen(true)} data-testid="mobile-menu-btn" aria-expanded={open} aria-label="Open menu">
