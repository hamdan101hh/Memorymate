...
import { Brain, UserCheck, ShieldCheck, HeartHandshake, Phone, ArrowRight, ArrowLeft, Check } from "lucide-react";
...
export default function Onboarding() {
  ...
  const steps = [
    {
      icon: Brain, color: "bg-sky-600",
      title: "Welcome to MemoryMate",
      body: "MemoryMate helps you save memories, reminders, appointments, and important notes in one simple place.",
      content: null,
    },
    {
      icon: user?.role === "patient" ? UserCheck : HeartHandshake, color: "bg-emerald-600",
      title: user?.role === "patient" ? "Using this for yourself" : "Caring for someone",
      body: user?.role === "patient"
        ? "Your account is set up for you. Your family can join as caregivers to help."
        : "Your caregiver account is connected to the person you care for. You can manage their reminders and care details.",
      content: null,
    },
    {
      icon: ShieldCheck, color: "bg-sky-600",
      title: "Privacy and consent",
      body: "Recordings and notes may be processed by AI to create simple summaries and reminders. Your information stays private.",
      content: (
    ...
    },
    {
      ...
      body: "MemoryMate is supportive, not diagnostic. It does not diagnose, treat, cure, or replace professional medical advice, emergency services, or clinical care.",
      content: null,
    },
    {
      icon: Phone, color: "bg-rose-600",
      title: "Add an emergency contact",
      body: "This is the trusted person to reach quickly. You can change it anytime in Settings.",
      content: (
      ...
      <main className="flex-1 flex items-center justify-center px-5 pb-10">
        <div className="w-full max-w-md text-center mm-fade-up" key={step}>
          ...
          <div className="mt-10 flex items-center gap-3">
            ...
            {!last ? (
              <Button onClick={() => canNext && setStep((x) => x + 1)} disabled={!canNext}
                className="flex-1 h-13 py-3 rounded-xl bg-sky-600 hover:bg-sky-700 text-base" data-testid="onboarding-next-btn">
